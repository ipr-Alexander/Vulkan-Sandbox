...
#include "rendering/uniformBuffer.hh"
...
void HelloTriangleApplication::createCommandPool()
{
    ...
    if (vkCreateCommandPool(device, &transferPoolInfo, nullptr, &commandTransferPool) != VK_SUCCESS)
    {
        throw std::runtime_error("Failed to create command pool!");
    }
}
...
void HelloTriangleApplication::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
{
    ...
    {
        throw std::runtime_error("Failed to begin recordin command buffer!");
    }
...
bool HelloTriangleApplication::isDeviceSuitable(VkPhysicalDevice device)
{
    ...
    return indices.isComplete() && extensionSupport && swapChainAdequate; // deviceProperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU && deviceFeatures.geometryShader;
}
...
void HelloTriangleApplication::generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels)
{
    ...
    for (uint32_t i = 1; i < mipLevels; i++)
    {
        ...
        // NOTE: Beware if you are using a dedicated transfer queue: vkCmdBlitImage must be submitted to a queue with graphics capability.
...
void HelloTriangleApplication::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels)
{
    ...
    barrier.image = image;
    if (newLayout == VK_IMAGE_LAYOUT_DEPTH_READ_ONLY_STENCIL_ATTACHMENT_OPTIMAL)
    {
